...
const stayInTouch = ({state, actions, libraries}) => {
    ...
    useEffect( () => {
        if(state.theme.lang === "en") {
            actions.source.fetch("/projet-arramat")
        }
   
        else {
            actions.source.fetch("/es-projet-arramat")
        }
    }, [])

    const Html2react = libraries.html2react.Component;

    const contentForm = state.theme.lang === "en" ? state.source.page[1516] : state.source.page[1308];
