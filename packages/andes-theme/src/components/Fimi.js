...
const stayInTouch = ({state, actions, libraries}) => {
    ...
    useEffect( () => {
        if(state.theme.lang === "en") {
            actions.source.fetch("/projet-fimi")
        }
   
        else {
            actions.source.fetch("/es-projet-fimi")
        }
    }, [])

    const Html2react = libraries.html2react.Component;

    const contentForm = state.theme.lang === "en" ? state.source.page[1524] : state.source.page[1312];
